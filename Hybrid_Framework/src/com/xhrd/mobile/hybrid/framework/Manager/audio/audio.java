...
public class audio extends PluginBase {
    ...
    private static final String JSON_PLAYER =
            ...
            "play:function(sucFunc, errFunc){" +
                "exec('RDCloud://com.xhrd.mobile.hybridframework.framework.Manager.audio.audio/playerCall/'+this.hostId, ['play',this.id, sucFunc, errFunc], false, false);" +
                //"rd.AudioManager.playerCall('play', %d, ja2sa(arguments));" +
            "}, " +
            "setRoute:function(){" +
                "if (arguments == null || arguments.length == 0)" +
                    "return; " +
                    "exec('RDCloud://com.xhrd.mobile.hybridframework.framework.Manager.audio.audio/playerCall/'+this.hostId, ['setRoute',this.id,arguments[0]], false, false);" +
                    //"rd.AudioManager.playerCall('setRoute', %d, ja2sa(arguments));" +
            "}, " +
            "seekTo:function(){" +
            	"if (arguments == null || arguments.length == 0)" +
                	"return; " +
                "exec('RDCloud://com.xhrd.mobile.hybridframework.framework.Manager.audio.audio/playerCall/'+this.hostId, ['seekTo',this.id,arguments[0]], false, false);" +
                //"rd.AudioManager.playerCall('seekTo', %d, ja2sa(arguments));" +
            "}, " +
            "getMode:function(){" +
                //"var ret = rd.AudioManager.playerCall('getMode', %d, ja2sa(arguments));" +
                "var ret = exec('RDCloud://com.xhrd.mobile.hybridframework.framework.Manager.audio.audio/playerCall/'+this.hostId, ['getMode',this.id], true, false);" +
                "return parseInt(ret);" +
            "}, " +
            "pause:function(){" +
                "exec('RDCloud://com.xhrd.mobile.hybridframework.framework.Manager.audio.audio/playerCall/'+this.hostId, ['pause',this.id], false, false);" +
                //"rd.AudioManager.playerCall('pause', %d, ja2sa(arguments));" +
            "}, " +
            "resume:function(){" +
                "exec('RDCloud://com.xhrd.mobile.hybridframework.framework.Manager.audio.audio/playerCall/'+this.hostId, ['resume',this.id], false, false);" +
                //"rd.AudioManager.playerCall('resume', %d, ja2sa(arguments));" +
            "}, " +
            "getDuration:function(){" +
                "var size = exec('RDCloud://com.xhrd.mobile.hybridframework.framework.Manager.audio.audio/playerCall/'+this.hostId, ['getDuration',this.id], true, false);" +
                "console.log('size: ' + size);" +
                "return size;" +
        	"}, " +
        	"getPosition:function(){" +
                "var time = exec('RDCloud://com.xhrd.mobile.hybridframework.framework.Manager.audio.audio/playerCall/'+this.hostId, ['getPosition',this.id], true, false);" +
                "return time;" +
    		"}, " +
            "isPlaying:function(){" +
                "var b = exec('RDCloud://com.xhrd.mobile.hybridframework.framework.Manager.audio.audio/playerCall/'+this.hostId, ['isPlaying',this.id], true, true);" +
                "return b == '1';" +
    		"}, " +
            "stop:function(){" +
                "exec('RDCloud://com.xhrd.mobile.hybridframework.framework.Manager.audio.audio/playerCall/'+this.hostId, ['stop',this.id], false, false);" +
