...
public class SysEventReceiver extends BroadcastReceiver {
    ...
    public SysEventReceiver(EventBus listener, HybridView view) {
        ...
        if (Intent.ACTION_DATE_CHANGED.equals(action)) {
            ...
            switch (chargePlug) {
                case BatteryManager.BATTERY_PLUGGED_AC:
                    mListener.sendEvent(mView,new String[]{EventBus.BATTER_STATE_CHANGED, String.valueOf(EventBus.BATTERY_STATE_TYPE_PLUGGED_AC)});
                    break;
                case BatteryManager.BATTERY_PLUGGED_USB:
                    mListener.sendEvent(mView,new String[]{EventBus.BATTER_STATE_CHANGED,String.valueOf(EventBus.BATTERY_STATE_TYPE_PLUGGED_USB)});
                    break;
                case BatteryManager.BATTERY_PLUGGED_WIRELESS:
                    mListener.sendEvent(mView,new String[]{EventBus.BATTER_STATE_CHANGED,String.valueOf(EventBus.BATTERY_STATE_TYPE_PLUGGED_WIRELESS)});
                    break;
            }
...
//            mListener.sendEvent(mView,new String[]{EventListener.BATTER_STATE_CHANGED, String.valueOf(EventListener.BATTERY_STATE_TYPE_CHARGING)});
            ...
            if(netInfo != null && netInfo.isAvailable()) {
                ...
                if(netInfo.getType()==ConnectivityManager.TYPE_WIFI) {
                    //WiFi网络
                    mListener.sendEvent(mView,new String[]{EventBus.NETWORK_STATE_CHANGED, String.valueOf(EventBus.NETWORK_STATE_TYPE_WIFI)});
                } else if(netInfo.getType()==ConnectivityManager.TYPE_ETHERNET) {
                    //有线网络
                    mListener.sendEvent(mView,new String[]{EventBus.NETWORK_STATE_CHANGED, String.valueOf(EventBus.NETWORK_STATE_TYPE_ETHERNET)});
                } else if(netInfo.getType()==ConnectivityManager.TYPE_MOBILE) {
                    //3g网络
                    mListener.sendEvent(mView,new String[]{EventBus.NETWORK_STATE_CHANGED, String.valueOf(EventBus.NETWORK_STATE_TYPE_MOBILE)});
                } else if(netInfo.getType()==ConnectivityManager.TYPE_VPN) {
                    //vpn网络
                    mListener.sendEvent(mView,new String[]{EventBus.NETWORK_STATE_CHANGED, String.valueOf(EventBus.NETWORK_STATE_TYPE_VPN)});
                } else if(netInfo.getType()==ConnectivityManager.TYPE_WIMAX) {
                    //WIMAX网络
                    mListener.sendEvent(mView,new String[]{EventBus.NETWORK_STATE_CHANGED, String.valueOf(EventBus.NETWORK_STATE_TYPE_WIMAX)});
                }
            } else {
                //网络断开
                mListener.sendEvent(mView,new String[]{EventBus.NETWORK_STATE_CHANGED, String.valueOf(EventBus.NETWORK_STATE_TYPE_NONE)});
            }
        }
    }
}
